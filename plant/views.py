...
def view_plant_repair(request):
    ...
    # # создаем пагинатор
    posts_per_page = request.GET.get('posts_per_page', 50)
...
def view_plant_task(request):
    ...
    # # создаем пагинатор
    try:
        posts_per_page = int(request.GET.get('posts_per_page', 50))
    except ValueError:
        posts_per_page = 10
...
def registration_task(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if error is None:
                # print(employee.id,task_object.id)
...
def edit_employee(request, pk):
    ...
    if request.method == 'POST':
        ...
        # print(employee.brigade, brigades_id)
        if form.is_valid():
            # print('форма валидна')
